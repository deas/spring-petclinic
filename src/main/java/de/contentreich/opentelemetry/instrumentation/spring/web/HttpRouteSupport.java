...
final class HttpRouteSupport {
	...
	void onFilterInit(FilterConfig filterConfig) {
		...
		if (servlet != null) {
			...
			((ConfigurableWebApplicationContext) context).addApplicationListener(new WebContextRefreshListener());
		}
	}

	// we can't retrieve the handler mappings from the DispatcherServlet in the onRefresh
	// listener,
	// because it loads them just after the application context refreshed event is
	// processed
	// to work around this, we're setting a boolean flag that'll cause this filter to load
	// the
	...
	/*
	 * boolean hasMappings() { if (contextRefreshTriggered.compareAndSet(true, false)) {
	 * // reload the handler mappings only if the web app context was recently refreshed
	 * DispatcherServlet dispatcherServlet = this.dispatcherServlet; if (dispatcherServlet
	 * != null) { List<HandlerMapping> mappings = dispatcherServlet.getHandlerMappings();
	 * if (mappings != null) { setHandlerMappings(mappings); } } } return handlerMappings
	 * != null; }
	 */
	...
	private void setHandlerMappings(List<HandlerMapping> mappings) {
		...
		for (HandlerMapping mapping : mappings) {
			// Originally we ran findMapping at the very beginning of the request. This
			// turned out to have
			// application-crashing side-effects with grails. That is why we don't add all
			// HandlerMapping
			// classes here. Although now that we run findMapping after the request, and
			// only when server
			// span name has not been updated by a controller, the probability of bad
			// side-effects is much
	...
	String getHttpRoute(Context context, HttpServletRequest request) {
		...
		try {
			if (findMapping(request)) {
				...
				// Let the parent span resource name be set with the attribute set in
				// findMapping.
			...
			// mimic spring DispatcherServlet and restore the previous value of
			// PATH_ATTRIBUTE
			if (parsePath) {
	...
	/**
	 * When a HandlerMapping matches a request, it sets
	 * HandlerMapping.BEST_MATCHING_PATTERN_ATTRIBUTE as an attribute on the request. This
	 * attribute set as the HTTP route.
	 */
